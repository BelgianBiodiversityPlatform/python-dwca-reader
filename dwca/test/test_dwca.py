...
class Test(unittest.TestCase):
    ...
    BASIC_ARCHIVE_PATH = os.path.join(os.path.dirname(__file__),
                           './sample_files/dwca-simple-test-archive.zip')
    NOHEADERS1_PATH = os.path.join(os.path.dirname(__file__),
                           './sample_files/dwca-noheaders-1.zip')
    NOHEADERS2_PATH = os.path.join(os.path.dirname(__file__),
                           './sample_files/dwca-noheaders-2.zip')
    ...
    def test_ignore_header_lines(self):
        ...
        with DwCAReader(self.NOHEADERS1_PATH) as dwca:
            # This file has two real lines, without headers (specified in meta.xml)
            self.assertEqual(2, len([l for l in dwca.each_line()]))

        with DwCAReader(self.NOHEADERS2_PATH) as dwca:
            # This file has two real lines, without headers (nothing specified in meta.xml)
            self.assertEqual(2, len([l for l in dwca.each_line()]))

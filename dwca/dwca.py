...
class DwcALine:
    ...
    def __init__(self, core_line, core_meta):
        ...
        # core_meta is a beautifulsoup object containing the <core> node (and content) from the metaxml 
        separator = core_meta['fieldsterminatedby'].decode("string-escape")
        fields = core_line.split(separator) # fields is list of the line's content

        # TODO: Consistency chek ?? fields length should be : # of fields described in core_meta + 2 (id and \n)
        ...
        for f in core_meta.findAll('field'):
            # if field by default, we can find its value directly in <field> attribute
...
class DwcAReader:
    def __init__(self, path):
        ...
        # Load the (scientific) metadata file and store its representation in metadata attribute for future use.
        self.metadata = self._parse_metadata_file()
        
        self._core_fhandler = codecs.open(self._get_core_filename(), mode='r', encoding=self._get_core_encoding(), errors='replace')

    def _create_temporary_folder(self):
        return mkdtemp()[1]

    def _parse_metadata_file(self):
        """Loads the archive (scientific) Metadata file, parse it with BeautifulSoup and return its content"""
        
        # This method should be called only after ._metaxml attribute is set
        # because the name/path to metadat file is stored in the "metadata" attribute of the "archive" tag
        metadata_file = self._get_metadata_filename()
        return self._parse_xml_included_file(metadata_file)

    def _parse_metaxml_file(self):
        """Loads the meta.xml, parse it with BeautifulSoup and return its content"""
        return self._parse_xml_included_file('meta.xml')
        
    def _parse_xml_included_file(self, relative_path):
        """Loads, parse with BeautifulSoup and returns XML file located at relative_path"""
    ...
    def _unzip(self):
        ...
        ZipFile(self.archive_path, 'r').extractall(unzipped_folder)#TODO: check content of file!!!! It may, for example contains absolute path (see zipfile doc)
        return unzipped_folder 

    def cleanup_temporary_folder(self):
        rmtree(self._unzipped_folder, False) #TODO: directory empty but still seems to be present (and undeletable if I add rmdir on the next line, investigate.

    def get_core_type(self):
        return self._metaxml.core['rowtype']

    def core_contains_term(self, term_url):
        """Takes a tdwg URL as a parameter and check if field exists for this concept in the core file"""
    ...
    def _get_core_filename(self):
        return self._unzipped_folder + '/' + self._metaxml.core.files.location.string
